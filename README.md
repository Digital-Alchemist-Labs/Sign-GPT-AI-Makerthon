...
### 주요 기능:
- **기능 A**: 수어 인식하여 키포인트를 추출 후, 해당되는 단어를 LLM서버에 전달
- **기능 B**: LLM서버를 통해 입력된 단어들을 질문으로 만들어 GPT에게 전달 후 받은 답변을 데이터베이스에서 해당되는 수어 영상을 프론트에 전달
- **기능 C**: 전달 받은 수어 답변 영상을 연속적으로 출력
